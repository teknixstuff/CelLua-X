...
function callmods(name)
    if love.filesystem.isFused() then
        local diriter = love.filesystem.getDirectoryItems("base/mods/")
        for k,fname in ipairs(diriter) do
            if love.filesystem.isFile("base/mods/" .. fname .. "/" .. name .. ".lua") then
                love.filesystem.load("base/mods/" .. fname .. "/" .. name .. ".lua")()
            end
        end
    end
end
...
placecells = true
width,height = 35,20
newwidth,newheight = 35,20
goid = 0
savedid = 0
delay,tpu,volume,svolume,border,uiscale,newuiscale = .2,1,1,1,2,2,2
bordercells = {1,41,12,205,51,141,150,151,152,126,176,"wrap"}
cam = {x=735,y=500,tarx=735,tary=500,zoom=40,tarzoom=40,zoomlevel=5}
...
paused = false
inmenu = false
puzzle,draggedcell,erasebg = nil,nul,nil
dodebug,subticking,subtick,fancy = false,false,1,true
mobile = false
if love.system.getOS() == 'iOS' or love.system.getOS() == 'Android' then
	uiscale,newuiscale = 1.35,1.35
	cam.zoom,cam.tarzoom,cam.zoomlevel = 20,20,4
	mobile = true
end
portals = {}
reverseportals = {}
...
function NewTex(val,key,modname)
	if key then
		if modname then
			tex[key] = love.graphics.newImage("base/mods/"..modname.."/"..val..".png")
		else
			tex[key] = love.graphics.newImage("resources/textures/"..val..".png")
		end
		texsize[key] = {w=tex[key]:getWidth(),h=tex[key]:getHeight(),w2=tex[key]:getWidth()*.5,h2=tex[key]:getHeight()*.5}
	else
		if modname then
			table.insert(tex,love.graphics.newImage("base/mods/"..modname.."/"..val..".png"))
		else
			table.insert(tex,love.graphics.newImage("resources/textures/"..val..".png"))
		end
...
currmusic = 4
destroysound = love.audio.newSource("resources/destroy.ogg", "static")
unlocksound = love.audio.newSource("resources/unlock.ogg", "static")
movesound = love.audio.newSource("resources/move.ogg", "static")
rotatesound = love.audio.newSource("resources/rotate.ogg", "static")
infectsound = love.audio.newSource("resources/infect.ogg", "static")
coinsound = love.audio.newSource("resources/coin.ogg", "static")
beep = love.audio.newSource("resources/beep.wav", "static")
music = love.audio.newSource("resources/scattered cells.ogg", "stream")
music2 = love.audio.newSource("resources/stepping stones.ogg", "stream")
music3 = love.audio.newSource("resources/seen sights.ogg", "stream")
music4s = love.audio.newSource("resources/part222.wav", "stream")
music4 = love.audio.newSource("resources/Cell_Machine_Remix.mp3", "stream")
music5s = love.audio.newSource("resources/The_Synth_Cell_intro.mp3", "stream")
music5 = love.audio.newSource("resources/The_Synth_Cell_main.mp3", "stream")
music6 = love.audio.newSource("resources/remix.wav", "stream")
music7 = love.audio.newSource("resources/Remix3.wav", "stream")
music8 = love.audio.newSource("resources/z_and_x.wav", "stream")
...
music7:setVolume(1)
music8:setVolume(1)
destroysound:setVolume(1)
unlocksound:setVolume(1)
movesound:setVolume(4)
rotatesound:setVolume(.5)
infectsound:setVolume(3)
coinsound:setVolume(1)
beep:setVolume(1)
love.audio.play(music4s)
...
--end miscellaneous setup

local font = love.graphics.newFont("resources/nokiafc22.ttf",8)
...
local bgsprites = love.graphics.newSpriteBatch(tex[0])
...
local function NewButton(x,y,w,h,icon,key,name,desc,onclick,ishold,enabledwhen,alignment,rot,color,hovercolor,clickcolor)
...
local propertynames = {}
local propertiesopen = 0
local function MakePropertyMenu(properties,b)
	propertynames = {}
...
for i=0,#lists do 
	local list = lists[i]
	NewButton(i*50+6,6,40,40,list.icon,"list"..i,list.name,list.desc,function() openedtab = openedtab == i and -1 or i; openedsubtab = -1; propertiesopen = 0 end,false,function() return not puzzle end,"bottomleft", hudrotation)
	for j=1,#list.cells do
		...
		if type(cell) == "table" then
			...
			for k=1,#cell do
				...
				local b = NewButton(i*50+16+x*20,j*20+34+y*20,20,20,tex[subcell] and subcell or "X","list"..i.."sublist"..j.."cell"..subcell,cellinfo[subcell].name,cellinfo[subcell].desc,function(b) openedsubtab = -1; openedtab = -1; propertiesopen = 0; SetSelectedCell(subcell,b) end,false,function() return not puzzle and openedtab == i and openedsubtab == j end,"bottomleft", hudrotation)
			end
		else
...
function DecodeKX0(code)
	local currentspot = 0
	local currentcharacter = 4 --start right after KX0
	local storedstring = ""
	TogglePause(true)
	isinitial = true
	undocells = {}
...
function DecodeKX0P(code)
	levels = string.sub(code,6):split("|")
	level = nil
	mainmenu = 2
	puzzle = true
	inmenu = false
...
function LoadWorld()
	LoadCode(love.system.getClipboardText(),true)
end

function LoadCode(txt,dobeep)
	local dobeep = dobeep or false
	if string.sub(txt,1,2) == "V3" then
		DecodeV3(txt)
		if dobeep then Play(beep) end
	...
	elseif string.sub(txt,1,2) == "K3" then
		DecodeK3(txt)
		if dobeep then Play(beep) end
	elseif string.sub(txt,1,4) == "KX0P" then
		DecodeKX0P(txt)
		if dobeep then Play(beep) end
	elseif string.sub(txt,1,3) == "KX0" then
		DecodeKX0(txt)
		if dobeep then Play(beep) end
	else
		Play(destroysound)
...
function SaveWorld()
	love.system.setClipboardText(SaveCode())
	Play(beep)
end
...
local bactive = function() return inmenu and inmenu ~= 2 and not winscreen and not mainmenu end
local bactive_or_browse = function() return inmenu and not winscreen and not mainmenu end
local optionbactive = function() return inmenu and inmenu ~= 2 and not winscreen and not mainmenu or mainmenu == 3 end
local strictbactive = function() return inmenu and inmenu ~= 2 and not level and not mainmenu end
local stricterbactive = function() return inmenu and inmenu ~= 2 and not puzzle and not mainmenu end
...
NewButton(-150,100,40,40,2,"closemenu","Close Menu",nil,function() inmenu = false Play(beep) end,false,bactive_or_browse,"center")
...
NewButton(-50,100,40,40,9,"resetpuzzlemenu","Reset Puzzle","Resets the puzzle to how it was in the beginning.",function() level = level - 1; NextLevel() end,false,function() return level and inmenu and inmenu ~= 2 end,"center")
...
NewButton(50,100,40,40,"mode","loadlvl","Load & Edit Level","Fetches from clipboard.\nLoads V3, K1-K3, and KX0 codes.",function() LoadWorld(); puzzle = false end,false,strictbactive,"center")
NewButton(100,100,40,40,"puzzle","puzzleloadlvl","Load Level as Puzzle","Fetches from clipboard.\nLoads V3, K1-K3, KX0, and KX0P codes.",function() LoadWorld(); puzzle = true; SetSelectedCell("eraser") end,false,strictbactive,"center")
NewButton(150,100,40,40,"delete","tomainmenu","Back to Level Select",nil,function() mainmenu = 2; puzzle = true; inmenu = false; Play(beep) end,false,function() return puzzle and inmenu end,"center",nil,{1,.5,.5,.5},{1,.5,.5,1},{.5,.25,.25,1})
if love.system.getOS() == 'Windows' then
	NewButton(150,100,40,40,"cloud","levelbrowser","Browse levels",nil,function() inmenu = 2 end,false,function() return not puzzle and inmenu and inmenu ~= 2 end,"center",nil,{1,1,1,.5},{1,1,1,1},{.5,.5,.5,1})
end
...
NewButton(0,-61,300,10,"pix","volumeslider",nil,nil,function() volume = math.round((love.mouse.getX()/uiscale-centerx/uiscale+150)/3)*.01 music:setVolume(volume) music2:setVolume(volume) music3:setVolume(volume) music4s:setVolume(volume) music4:setVolume(volume) music5s:setVolume(volume) music5:setVolume(volume) music6:setVolume(volume) music7:setVolume(volume) music8:setVolume(volume) end,true,optionbactive,"center",nil,{.25,.25,.25,1},{.25,.25,.25,1},{.25,.25,.25,1})
...
NewButton(50,25,20,20,"music","musicbtn","Change Music",nil,function() if currmusic == 1 then music:stop() music2:play() currmusic = 2 elseif currmusic == 2 then music2:stop() music3:play() currmusic = 3 elseif currmusic == 3 then music4s:play() music3:stop() currmusic = 4 elseif currmusic == 4 then music5s:play() music4s:stop() music4:stop() currmusic = 5 elseif currmusic == 5 then music6:play() music5s:stop() music5:stop() currmusic = 6 elseif currmusic == 6 then music7:play() music6:stop() currmusic = 7 elseif currmusic == 7 then music8:play() music7:stop() currmusic = 8 else music:play() music8:stop() currmusic = 1 end end,false,optionbactive,"center",nil,{.5,.5,.5,1},{.75,.75,.75,1},{.25,.25,.25,1})
...
NewButton(-130,-75,75,25,"pix","goidbtn",nil,nil,function() if not puzzle then typing = 3 end end,false,function() return inmenu == 2 end,"center",nil,{.25,.25,.25,1},{.25,.25,.25,1},{.25,.25,.25,1})
NewButton(-100,-75,40,40,"clouddown","cloudload","Load level",nil,function() end,false,function() return inmenu == 2 end,"center",nil,{1,1,1,.5},{1,1,1,1},{.5,.5,.5,1})
...
NewButton(80,100,60,60,2,"startgamebtn","Sandbox",nil,function() mainmenu = false; newwidth = 35; newheight = 20; border = 2; delay = .2; tpu = 1; title,subtitle = "",""; ClearWorld(); puzzle = false; level = nil ResetCam() Play(beep) end,false,mmenu1,"center")
NewButton(20,20,40,40,"delete","backtomain","Go Back",nil,function() mainmenu = false; newwidth = 35; newheight = 20; border = 2; delay = .2; tpu = 1; title,subtitle = "",""; ClearWorld(); puzzle = false; level = nil ResetCam() Play(beep) end,false,exitbtn,"topleft")
...
for i=0,34 do
	NewButton(50*(i%xamnt)-xoff,50*math.floor(i/xamnt)-yoff,40,40,"checkoff","topuzzle"..i+1,nil,nil,function() level = i; NextLevel() ResetCam() Play(beep) end,false,function() return mmenu2() and (i < #levels) end,"center")
end
...
function IsNonexistant(cell,dir,x,y)	--act like empty space
	return cell.id == 0 or cell.id == 116 or cell.id == 117 or cell.id == 118 or cell.id == 119 or cell.id == 120 or cell.id == 121 or cell.id == 122 or cell.id == 223 or cell.id == "wrap" or (cell.id >= 1000 and cellcalls[cell.id].nonsolid)
end
...
local function RotateCell(x,y,rot,dir,large)
	local cell = GetCell(x,y)
...
local function RotateCellTo(x,y,rot,dir)
	local cell = GetCell(x,y)
	if cell.locked then return end
	if cell.id == 105 and updatekey ~= cell.updatekey then
		cell.rot = rot
...
local function FlipCell(x,y,rot,dir)
	local cell = GetCell(x,y)
	if cell.locked then return end
	if cell.id == 105 and updatekey ~= cell.updatekey then
		cell.updatekey = updatekey
...
local function FreezeCell(x,y,dir,large)
	local cell = GetCell(x,y)
...
local function ThawCell(x,y,dir)
	local cell = GetCell(x,y)
...
local function ProtectCell(x,y,dir,size)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.protected = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function LockCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.locked = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function ClampCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.clamped = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function LatchCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.latched = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function SealCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.sealed = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function BoltCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
	cell.bolted = true
	if cell.id == 105 and updatekey ~= cell.updatekey then
...
local function ReinforceCell(x,y,dir)
	local cell = GetCell(x,y)
	if IsNonexistant(cell,dir,x,y) then return end
...
local function GravitizeCell(x,y,dir,gdir)
	local cell = GetCell(x,y)
...
local function StickCell(x,y,dir,gdir)
	local cell = GetCell(x,y)
...
local function DoQuantumEnemy(cell,vars)
...
function HandleNudge(cell,dir,x,y,vars)
	...
	local side = ToSide(rot,dir)
	if id >= 1000 then
		return cellcalls[id].HandleNudge(cell,dir,x,y,vars)
	end
...
function HandlePush(force,cell,dir,x,y,vars)
	...
	if id >= 1000 then
		return cellcalls[id].HandlePush(force,cell,dir,x,y,vars)
	end
...
function HandleGrasp(force,cell,dir,x,y,vars)
	...
	local side = ToSide(rot,dir)
	if id >= 1000 then
		return cellcalls[id].HandleGrasp(cell,dir,x,y,vars)
	end
	if cell.sealed then
		vars.ended = true
		return force
...
function HandlePull(force,cell,dir,x,y,vars)
	...
	local side = ToSide(rot,dir)
	if id >= 1000 then
		return cellcalls[id].HandlePull(force,cell,dir,x,y,vars)
	end
	if cell.sticky and not vars.checkonly then
		if not vars.sticking then stickkey = stickkey + 1 end
		vars.sticking = true
...
function HandleSwap(cell,dir,x,y,vars)
	...
	local side = ToSide(rot,dir)
	if id >= 1000 then
		return cellcalls[id].HandleSwap(cell,dir,x,y,vars)
	end
...
local function DoCheater(x,y,cell)
	cell.updated = true
	if cell.id == 199 then
		local dir = cell.rot
		if dir == 0 then x = x-1 elseif dir == 2 then x = x+1
...
local function DoThawer(x,y,cell)
	local neighbors = GetNeighbors(x,y)
	for k,v in pairs(neighbors) do
		ThawCell(v[1],v[2],k)
	end
...
local function DoFreezer(x,y,cell)
	if cell.id == 286 then
		local neighbors = GetSurrounding(x,y)
		for k,v in pairs(neighbors) do
			FreezeCell(v[1],v[2],k,true)
...
local function DoEffectGiver(x,y,cell)
	if cell.id == 43 then
		local neighbors = GetSurrounding(x,y)
		for k,v in pairs(neighbors) do
...
local function DoDegravitizer(x,y,cell)
	local neighbors = GetNeighbors(x,y)
	for k,v in pairs(neighbors) do
...
local function DoTimewarper(x,y,cell,dir)
	local c = {rot = 0, dir = 0}
...
local function DoTransformer(x,y,cell,dir)
...
local function DoHMirror(x,y,cell)
...
local function DoVMirror(x,y,cell)
...
local function DoDMirror(x,y,cell)
...
local function DoIntaker(x,y,cell,dir)
...
local function DoShifter(x,y,cell,dir)
...
local function DoCreator(x,y,cell)
	if cell.vars[1] then
		for i=0,3 do
			local cx,cy = x,y
...
local function DoMemory(x,y,cell)
	cell.updated = true
...
local function DoSuperGenerator(x,y,cell)
	cell.updated = true
	local gencells = {}
...
local function DoSuperReplicator(x,y,cell)
	cell.updated = true
	local gencells = {}
...
local function DoReplicator(x,y,cell,dir)
...
local function DoFlipper(x,y,cell)
	local neighbors = GetNeighbors(x,y)
	if cell.id == 30 then
		for k,v in pairs(neighbors) do
...
local function DoRotator(x,y,cell)
	local neighbors = GetNeighbors(x,y)
	if cell.id == 9 then
		for k,v in pairs(neighbors) do
...
local function DoGear(x,y,cell)
	if cell.id == 18 then
		local neighbors = GetSurrounding(x,y)
		local jammed
		for k,v in pairs(neighbors) do
...
local function DoRedirector(x,y,cell)
	local neighbors = GetNeighbors(x,y)
	if cell.id == 17 then
		for k,v in pairs(neighbors) do
...
local function DoInertia(x,y,cell,dir)
...
local function DoSuperImpulsor(x,y,dir)
...
local function DoImpulsor(x,y,dir)
	dir = (dir+2)%4
...
local function DoGrapulsor(x,y,cell,dir)
...
local function DoSuperRepulsor(x,y,dir)
	local cx,cy,cdir = NextCell(x,y,dir)
	while true do
...
local function DoTimeRepulsor(x,y,dir)
	for i=0,3 do
...
local function DoRepulsor(x,y,dir)
	if dir == 0 then x = x + 1 elseif dir == 2 then x = x - 1
	elseif dir == 1 then y = y + 1 elseif dir == 3 then y = y - 1 end
	PushCell(x,y,dir,{force=1,noupdate=true})
end

local function DoMagnet(x,y,cell)
	cell.updated = true
	local cx,cy,cdir = x,y,cell.rot
	if cell.rot == 0 then cx = x + 1 elseif cell.rot == 2 then cx = x - 1
	elseif cell.rot == 1 then cy = y + 1 elseif cell.rot == 3 then cy = y - 1 end
...
local function DoTermite(x,y,cell)
	cell.updated = true
	if cell.id == 306 then
		cell.rot = (cell.rot+1)%4
		for i=1,4 do
...
local function SliceCell(x,y,dir,vars)
	local cell = GetCell(x,y)
...
local function DoDriller(x,y,cell)
	cell.updated = true
...
local function DoPuller(x,y,cell)
	cell.updated = true
	local cx,cy = x,y
	if cell.rot == 0 then cx = x - 1 elseif cell.rot == 2 then cx = x + 1
	elseif cell.rot == 1 then cy = y - 1 elseif cell.rot == 3 then cy = y + 1 end
...
local function DoGrasper(x,y,cell)
	cell.updated = true
	if cell.id == 71 then
		GraspCell(x,y,cell.rot)
...
local function DoSuperPusher(x,y,cell)
	cell.updated = true
	local cx,cy,cdir = x,y,cell.rot
	while true do
		local nextx,nexty,nextdir = NextCell(cx,cy,cdir)
...
local function DoPusher(x,y,cell)
	cell.updated = true
...
local function DoSlicer(x,y,cell)
	cell.updated = true
	if cell.id == 115 then
		SliceCell(x,y,cell.rot)
	end
...
local function getAct(num,x,y,dir)
...
local function DoNudger(x,y,cell)
	cell.updated = true
	if cell.id == 114 then
...
local function DoCoinExtractor(x,y,cell)
	cell.updated = true
...
local function DoGate(x,y,cell)
...
local function SendGlunkiSignal(x,y,life)
	local neighbors = GetNeighbors(x,y)
	for k,v in pairs(neighbors) do
...
local function DoInfectious(x,y,cell)
	if cell.id == 123 then
...
local function DoPlayer(x,y,cell)
	cell.updated = true
	if held then
...
subticks = {
	--function() return RunOn(function(c) return not c.updated and c.id >= 1000 end, function(x,y,c) cellcalls[c.id].FirstTick(x,y,c) end, "", 0) end,
	...
	CheckEnemies,
	--function() return RunOn(function(c) return not c.updated and c.id >= 1000 end, function(x,y,c) cellcalls[c.id].FinalTick(x,y,c) end, "", 0) end,
}
...
function love.draw()
	if not music4s:isPlaying() and currmusic == 4 then
		music4:play()
	end
	if not music5s:isPlaying() and currmusic == 5 then
		music5:play()
	end
	...
	elseif mainmenu == 1 then
		...
		love.graphics.setColor(0,0,0,1)
		local t = "CelLua Machine X Alpha 3\nCreated by Tech Stuff\nBased on CelLua Machine Version 2.-1.5 PRE-RELEASE STUPID NOT FUL\nCelLua Machine Created by KyYay\nOriginal Cell Machine by Sam Hogan"
	...
	if inmenu and not winscreen and not mainmenu then
		...
		love.graphics.printf("CelLua Machine X",centerx,centery-127*uiscale,100,"center",0,scale,2*uiscale,50,8,skew)
		...
		love.graphics.printf("CelLua Machine X",centerx,centery-127*uiscale,100,"center",0,scale,2*uiscale,50,8,skew)
		love.graphics.setColor(1,1,1,1)
		love.graphics.printf("CelLua Machine X",centerx,centery-127*uiscale,100,"center",0,2*uiscale,2*uiscale,50,8,skew)
		love.graphics.setColor(0.5,0.5,0.5,1)
		if inmenu == 2 then
			local goid_str = goid
			if goid_str == 0 then goid_str = "" end
			if typing == 3 then love.graphics.print(goid_str.."_",centerx-160*uiscale,centery-82*uiscale,0,2*uiscale,2*uiscale) else love.graphics.print(goid_str,centerx-150*uiscale,centery-82*uiscale,0,2*uiscale,2*uiscale) end
		else
			...
			love.graphics.print("UI Scale: "..newuiscale*100 .."%",centerx-150*uiscale,300*winym-10*uiscale,0,uiscale,uiscale)
                ...
                love.graphics.setColor(rainbow(1))
		love.graphics.print("Alpha 3 (Live Build)",centerx-185*uiscale,300*winym+135*uiscale,0,uiscale,uiscale)
	...
	if subticking then
		love.graphics.print("Subtick: "..subtick.."/"..#subticks,2,2) 
	end
end
...
function love.keypressed(key)
	if typing then
		...
		elseif typing == 3 then
			if tonumber(key) then
				goid = tonumber(string.sub(tostring(goid)..key,1,6))
			elseif key == "backspace" then
				goid = tonumber(string.sub(tostring(goid),1,string.len(tostring(goid))-1)) or 0
			elseif key == "return" then
				if goid == 0 then
					local response = request.send("https://cellua-x-level-database.techstuff3.repl.co/addlevel", {
						method = "POST",
						data = {
							code = SaveCode()
						}
					...
					local response = request.send("https://cellua-x-level-database.techstuff3.repl.co/getlevel?id="..tostring(goid))
					LoadCode(response.body,true)
					goid = 0
				end
			end
		end
